...
export default class WordListRepository {
    ...
    /**
     * Pobiera losowe słowo z kolekcji wordlist.
     * @returns Obiekt IWordList lub null, jeśli kolekcja jest pusta.
     */
    static async getRandomWord(wordLength: number, language: string, level?: 'easy' | 'medium' | 'hard'): Promise<string | null> {
        ...
        const randomWords = await this.db.collection<IWordList>(WORDLIST_COLLECTION_NAME).aggregate([
            { $match: query },
            { $sample: { size: 1 } }
        ]).toArray();
    ...
    static async addWord(wordData: Omit<IWordList, '_id' | 'id'>): Promise<IWordList> {
        const result = await this.db.collection<IWordList>(WORDLIST_COLLECTION_NAME).insertOne(wordData as IWordList);
        return { ...wordData, _id: result.insertedId } as IWordList;
    }

    static async doesWordExist(word: string, language: string): Promise<boolean> {
        try {
            const result = await this.db.collection<IWordList>(WORDLIST_COLLECTION_NAME).findOne({ word, language });
            return !!result;
        } catch (error) {
    ...
    static async deleteWordsByLanguage(language: string): Promise<void> {
        try {
            await this.db.collection<IWordList>(WORDLIST_COLLECTION_NAME).deleteMany({ language });
